...
1. **Block Schedule**: 
   ...
   - Assign each resident to an Attending, Satellite (L&M), Satellite (Proton), or Elective rotation for the block. 
   - *Solver Rules*: Ensures each resident has exactly one assignment per block and that Satellite (L&M) is always covered by at least one resident. (Satellite (Proton) is optional).

2. **Call Schedule**:
   - Generates weekly call assignments.
   - *Solver Rules*: Respects PGY target call weeks based on the configuration. Prevents residents from taking call if they are assigned to Satellite (L&M), Satellite (Proton), or are on Vacation during that week.

3. **Inpatient Schedule**:
   - Generates daily weekday (Monday-Friday) inpatient coverage.
   - *Solver Rules*: Prevents assignment if the resident is on Vacation, on Satellite (L&M), on Satellite (Proton), or if their currently assigned Attending for that block has a clinic day on that specific weekday. It also prevents overlap with Cross Coverage assignments.
...
5. **Cross Coverage**:
   ...
   - *Solver Rules*: Assigns an available resident to cover the Attending's clinic. It prevents taking a resident who is on Vacation, assigned to Satellite (L&M) or Satellite (Proton), or already assigned to the Inpatient schedule on that day. It aims to distribute coverage shifts among remaining eligible residents.
